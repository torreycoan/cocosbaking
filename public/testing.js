...
async function go() {
  ...
  await page.click("#orderlJztPNZFQy9RszyrHGX8newpaymentstatus");
  await page.keyboard.press("P");
  await page.keyboard.press("Enter");
  await page.click("#lJztPNZFQy9RszyrHGX8 > td:nth-child(14) > button");
  //   // now filter to see if the update went correctly (should only show a one or two orders)
  await page.click("#orderstatusfilter");
  // gets it to pending acceptance/rejection
  await page.keyboard.press("P");
  await page.keyboard.press("Enter");
  await page.keyboard.press("Tab");
  // gets it to paid
  ...
  await page.click("#EqOyPgJNLm0XwhBapchf > td:nth-child(15) > button");
